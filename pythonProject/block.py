...
class Block(object):
    ...
    def __init__(self, shape, x, y, screen, color, rotate_en):
        ...
        for sh in shape:
            ...
            # Настраиваем атрибут вращения
        self.rotate_en = rotate_en
        # Настраиваем остальные переменные
        self.x = x
        self.y = y
        # Движение по координатам X,Y
        self.diffx = 0
        self.diffy = 0
        # Экран для рисования
        self.screen = screen
    ...
    def _update(self):
        for bl in self.shape:
            # Получить старые координаты и вычислить новые координаты x,y.
            # Все расчеты вращения выполняются в исходных координатах.
